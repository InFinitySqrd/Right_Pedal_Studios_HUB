...
public class RandomlyGenerateEnvironment : MonoBehaviour {
	...
	void Start () {
		...
		centre.transform.position = new Vector3(player.transform.position.x, player.transform.position.y - plane.transform.localScale.z * 1.45f, player.transform.position.z);
		...
		for (int i = 0; i < numPlanes; i++) {
			...
			Vector3 spawnPoint = new Vector3(centre.position.x, centre.position.y + plane.transform.localScale.z, centre.position.z);
